    ...
    if (withMentions)
      setTimeout(() => {
        if (e.key === '@') {
          ...
          if (!node && (!prevButton || prevButton === ' ' || currentTargetRef.current.textContent === '@')) {

            const caretPosition = editor.dom.doc.getSelection().anchorOffset;

            currentTargetRef.current.innerHTML = currentTargetRef.current.textContent.slice(0, caretPosition - countDogs(currentTargetRef.current.textContent, caretPosition)) +
              '<span class="last-mention">@</span>' + currentTargetRef.current.textContent.slice(caretPosition);
